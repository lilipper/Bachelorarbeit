# train_diffusion_classifier_cv.py
import os
import argparse
import csv
import numpy as np
...
def train_one_epoch(
    ...
    """
    One training epoch for the diffusion classifier:
      - THz -> RGB via trainable front
      - Encode to latents via VAE
      - Call eval_prob_adaptive_differentiable with a callable UNet that injects ControlNet residuals
      - Pool prompt errors to class errors; compute CE loss on logits = -class_errors * scale
    """
    ...
    for step, (vol, label, _) in enumerate(loader, start=1):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # Pool prompt errors to class errors per batch
...
# ----------------------- Main -----------------------
def main():
    ap = argparse.ArgumentParser(description="Diffusion Classifier (SD + ControlNet) with THz front, RSKF, and Final Eval")
