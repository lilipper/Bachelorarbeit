...
def interpolate_pos_embed_in_state(state_dict, model, key_candidates=("encoder.pos_embedding","pos_embed")):
    # finde den richtigen Key
    key = None
    for k in key_candidates:
        if k in state_dict:
            ...
            break
    if key is None:
        return state_dict  # nix zu tun

    posemb = state_dict[key]  # [1, old_num_tokens, C]
    if not hasattr(model, "patch_embed"):
        return state_dict

    # Ziel-Token-Anzahl aus dem Modell
    target = getattr(model, "pos_embed", None) or getattr(model, "encoder", None)
    try:
        target_num_tokens = model.encoder.pos_embedding.shape[1]
        target_posemb = model.encoder.pos_embedding
    except Exception:
        target_posemb = getattr(model, "pos_embed", None)
        target_num_tokens = None if target_posemb is None else target_posemb.shape[1]
    ...
    if gs_old * gs_old != grid.shape[1]:
        return state_dict  # keine quadratische Grid-Annahme möglich

    # neue Grid-Seitenlänge
    try:
        num_patches_new = model.patch_embed.num_patches
    except Exception:
        num_patches_new = target_num_tokens - 1
    gs_new = int(num_patches_new ** 0.5)
    if gs_new * gs_new != num_patches_new:
        return state_dict

    # [1, old^2, C] -> [1, C, old, old]
    grid = grid.reshape(1, gs_old, gs_old, posemb.shape[-1]).permute(0, 3, 1, 2)
    grid = F.interpolate(grid, size=(gs_new, gs_new), mode="bicubic", align_corners=False)
    grid = grid.permute(0, 2, 3, 1).reshape(1, gs_new*gs_new, posemb.shape[-1])

    state_dict[key] = torch.cat([cls_tok, grid], dim=1)  # [1, 1+new^2, C]
    return state_dict
...
def main():
    ...
    if args.final_eval:
        ...
        state_backbone = interpolate_pos_embed_in_state(state_backbone, backbone)
