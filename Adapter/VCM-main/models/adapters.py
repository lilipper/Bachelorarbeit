...
class Adapter_XL(nn.Module):
    ...
    def __init__(self, channels=[256,512,768], nums_rb=3, cin=64, ksize=3, sk=False, use_conv=True):
...
if __name__ == '__main__':
    ...
    adapter = Adapter_XL().cuda()

    # input = torch.rand(2,64,20,28,20).cuda()


    # features = adapter(input)

    # # for f in features:
    # #     print(f.shape)
        
    # for iter in range(len(features)):
    #     print(features.pop().shape)