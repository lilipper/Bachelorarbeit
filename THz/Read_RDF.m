...
tic
% Window data (here: Hamming window)
disp('Hamming window ...')

hamming_wind = hamming(NF);
data_cmp_win = bsxfun(@times, hamming_wind, data_complex_all);

clear data_complex_all;
toc;
tic
disp('FFT ...')

% Perform FFT and shift DC
up = 1;

% Use windowed data or not
data_final = fftshift(fft(data_cmp_win, NF*up), 1);
% data_final=fftshift(fft(data_complex_all),1);
...
% Use amplitudes
mag_final_all = abs(data_final).^2;
% Figure out maximum for normalization
max_mag_final = squeeze(max(mag_final_all,[], 1));
max_mag_final_all = max(mag_final_all, [], "all");
