...
while not(strcmp(name, 'end_of_var_list'))
    line_str = fgetl(fid);
    if strcmp(strtrim(line_str), 'end_of_var_list')
        break
    end

    % Jetzt sicher parsen
    tokens = textscan(line_str, '%s %f');
    if isempty(tokens{1}) || isempty(tokens{2})
        error(['Ungültige Zeile im Variablenblock: ', line_str]);
    end

    varname = tokens{1}{1};  % sicherer String
    value = tokens{2};

    if ~strcmp(varname, 'end_of_var_list')
        eval([varname ' = ' num2str(value) ';']);
        if save_data
            save(filename_mat, varname, '-append');
        end
    end
end
toc;
